...
import { Edit, Mail, Phone, FileText, CheckCircle, User, Trash2 } from "lucide-react";

const StudentDetailsModal = ({ student, customFields = [], onEdit, onClose, onDelete }) => {
    ...
    <div className="w-full">
      ...
      <div className="pt-16 px-6 text-center space-y-3">
        ...
        <div className="flex items-center justify-center gap-3">
          ...
          <button onClick={onEdit} className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-gray-900/10 hover:bg-gray-900/20 text-gray-800 dark:text-white transition">
            <Edit size={14} /> Edit
          </button>

          {onDelete && (
            <button onClick={onDelete} className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-red-50 hover:bg-red-100 text-red-700 transition">
      ...
      <div className="mt-8 px-4 pb-10 space-y-6">
        <div className="bg-white dark:bg-gray-900 rounded-xl shadow-md border p-6">
          ...
          <div className="space-y-4">
            <InfoRow icon={<Mail size={18} className="text-indigo-600" />} label="Email" value={student.email} />
            <InfoRow icon={<Phone size={18} className="text-indigo-600" />} label="Phone" value={student.phone} />
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl shadow-md border p-6">
          ...
          <div className="space-y-4">
            {customFields.map((f) => {
              ...
              const value = f.type === "checkbox" ? (v ? "Yes" : "No") : (v || "-");
              return <InfoRow key={f.id} icon={<FileText size={18} className="text-indigo-600" />} label={f.label} value={value} />;
            })}
          </div>
        </div>
      </div>
...
const InfoRow = ({ icon, label, value }) => (
  <div className="flex justify-between items-start gap-4">
    <div className="flex items-start gap-3">
      <div className="p-2 rounded-lg bg-indigo-50 dark:bg-indigo-900/30">{icon}</div>
      <div>
